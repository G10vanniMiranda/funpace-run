...
const emptyPartner: PartnerInput = { name: '', slug: '', partnerType: 'sports_advisory', discountPercentage: 10, athleteLimit: null, description: null, status: 'active' };
...
export function PartnersPanel({ adminKey }: { adminKey: string }) {
  ...
  const openEdit = (partner: AdminPartner) => {
    ...
    setDraft({ name: partner.name, slug: partner.slug, partnerType: partner.partnerType, discountPercentage: partner.discountPercentage, athleteLimit: partner.athleteLimit, description: partner.description, status: partner.status });
...
function PartnerTable({ partners, publicOrigin, copiedId, onCopy, onEdit, onStatus, onDelete }: { partners: AdminPartner[]; publicOrigin: string; copiedId: string; onCopy: (partner: AdminPartner) => void; onEdit: (partner: AdminPartner) => void; onStatus: (partner: AdminPartner) => void; onDelete: (partner: AdminPartner) => void }) {
  return <table className="w-full min-w-360 text-left"><thead className="border-b border-white/10 bg-black/40 text-[10px] uppercase tracking-widest text-zinc-500"><tr>{['Nome', 'Tipo', 'Slug', 'Desconto', 'Limite', 'Status', 'Criado em', 'Link exclusivo', 'Acoes'].map((label) => <th key={label} className="px-4 py-3">{label}</th>)}</tr></thead><tbody className="divide-y divide-white/8">{partners.map((partner) => <tr key={partner.id} className="hover:bg-white/2"><td className="px-4 py-4"><p className="font-bold">{partner.name}</p>{partner.description && <p className="mt-1 max-w-48 truncate text-xs text-zinc-500">{partner.description}</p>}</td><td className="px-4 py-4"><PartnerTypeBadge partnerType={partner.partnerType} /></td><td className="px-4 py-4 font-mono text-xs text-zinc-300">{partner.slug}</td><td className="px-4 py-4 font-mono font-bold text-brand">{partner.discountPercentage}%</td><td className="px-4 py-4 font-mono text-xs text-zinc-300">{partner.athleteLimit === null ? 'Ilimitado' : partner.athleteLimit}</td><td className="px-4 py-4"><StatusBadge status={partner.status} /></td><td className="px-4 py-4 text-xs text-zinc-400">{dateFormatter.format(new Date(partner.createdAt))}</td><td className="max-w-70 px-4 py-4"><p className="truncate font-mono text-[11px] text-zinc-500">{buildPartnerLink(publicOrigin, partner.slug)}</p><button type="button" onClick={() => void onCopy(partner)} className="mt-2 inline-flex items-center gap-1.5 text-[10px] font-black uppercase text-brand hover:text-white">{copiedId === partner.id ? <Check className="h-3.5 w-3.5" /> : <Clipboard className="h-3.5 w-3.5" />}{copiedId === partner.id ? 'Copiado' : 'Copiar link'}</button></td><td className="px-4 py-4"><ActionButtons partner={partner} onEdit={() => onEdit(partner)} onStatus={() => onStatus(partner)} onDelete={() => onDelete(partner)} /></td></tr>)}</tbody></table>;
}
