...
import { BarChart3, Check, Clipboard, Edit3, Loader2, Plus, Power, RefreshCcw, Search, Trash2, Users, X } from 'lucide-react';
import {
  ApiError,
...
import { buildPartnerLink, copyPartnerLink, slugifyPartnerName } from '../../lib/partners';
import type { AdminPartner, PartnerInput, PartnerStatus } from '../../types/partner';
import { PartnerDashboard } from './PartnerDashboard';

const emptyPartner: PartnerInput = { name: '', slug: '', discountPercentage: 10, description: null, status: 'active' };
...
export function PartnersPanel({ adminKey }: { adminKey: string }) {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const response = await getAdminPartners(adminKey, filters);
      setPartners(response.partners); setError('');
    } catch (requestError) {
  ...
  }, [adminKey, filters.name, filters.slug, filters.status]);
  ...
  const openEdit = (partner: AdminPartner) => {
    ...
    setDraft({ name: partner.name, slug: partner.slug, discountPercentage: partner.discountPercentage, description: partner.description, status: partner.status });
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      const response = editingId
        ? await updateAdminPartner(adminKey, editingId, draft)
        : await createAdminPartner(adminKey, draft);
      setDraft(null); setEditingId(''); setMessage(editingId ? 'Parceiro atualizado com sucesso.' : 'Parceiro criado com sucesso.');
      await load();
      if (!editingId) setPartners((current) => current.some((item) => item.id === response.partner.id) ? current : [...current, response.partner]);
    } catch (requestError) {
  ...
  return (
    <section className="space-y-4">
      <div className="flex flex-col justify-between gap-4 border border-white/10 bg-zinc-950 p-5 sm:flex-row sm:items-center">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.24em] text-brand">Assessorias esportivas</p>
      ...
      <div className="grid gap-3 border border-white/10 bg-zinc-950 p-4 md:grid-cols-[1fr_1fr_220px]">
        <FilterInput label="Buscar por nome" value={filters.name} onChange={(name) => setFilters({ ...filters, name })} />
        <FilterInput label="Buscar por slug" value={filters.slug} onChange={(slug) => setFilters({ ...filters, slug: slugifyPartnerName(slug) })} mono />
        <label className="text-xs font-bold text-zinc-400">Status
          <select value={filters.status} onChange={(event) => setFilters({ ...filters, status: event.target.value })} className="mt-1 min-h-11 w-full border border-white/10 bg-black px-3 text-sm text-white outline-none focus:border-brand">
...
function PartnerTable({ partners, publicOrigin, copiedId, onCopy, onEdit, onStatus, onDelete }: { partners: AdminPartner[]; publicOrigin: string; copiedId: string; onCopy: (partner: AdminPartner) => void; onEdit: (partner: AdminPartner) => void; onStatus: (partner: AdminPartner) => void; onDelete: (partner: AdminPartner) => void }) {
  return <table className="w-full min-w-300 text-left"><thead className="border-b border-white/10 bg-black/40 text-[10px] uppercase tracking-widest text-zinc-500"><tr>{['Nome', 'Slug', 'Desconto', 'Status', 'Criado em', 'Link exclusivo', 'Acoes'].map((label) => <th key={label} className="px-4 py-3">{label}</th>)}</tr></thead><tbody className="divide-y divide-white/8">{partners.map((partner) => <tr key={partner.id} className="hover:bg-white/2"><td className="px-4 py-4"><p className="font-bold">{partner.name}</p>{partner.description && <p className="mt-1 max-w-48 truncate text-xs text-zinc-500">{partner.description}</p>}</td><td className="px-4 py-4 font-mono text-xs text-zinc-300">{partner.slug}</td><td className="px-4 py-4 font-mono font-bold text-brand">{partner.discountPercentage}%</td><td className="px-4 py-4"><StatusBadge status={partner.status} /></td><td className="px-4 py-4 text-xs text-zinc-400">{dateFormatter.format(new Date(partner.createdAt))}</td><td className="max-w-70 px-4 py-4"><p className="truncate font-mono text-[11px] text-zinc-500">{buildPartnerLink(publicOrigin, partner.slug)}</p><button type="button" onClick={() => void onCopy(partner)} className="mt-2 inline-flex items-center gap-1.5 text-[10px] font-black uppercase text-brand hover:text-white">{copiedId === partner.id ? <Check className="h-3.5 w-3.5" /> : <Clipboard className="h-3.5 w-3.5" />}{copiedId === partner.id ? 'Copiado' : 'Copiar link'}</button></td><td className="px-4 py-4"><ActionButtons partner={partner} onEdit={() => onEdit(partner)} onStatus={() => onStatus(partner)} onDelete={() => onDelete(partner)} /></td></tr>)}</tbody></table>;
}
...
function PartnerCard({ partner, publicOrigin, copied, onCopy, onEdit, onStatus, onDelete }: { partner: AdminPartner; publicOrigin: string; copied: boolean; onCopy: () => void; onEdit: () => void; onStatus: () => void; onDelete: () => void }) {
  return <article className="space-y-4 p-4"><div className="flex items-start justify-between gap-3"><div><h3 className="font-bold">{partner.name}</h3><p className="mt-1 font-mono text-xs text-zinc-500">{partner.slug}</p></div><StatusBadge status={partner.status} /></div><div className="grid grid-cols-2 gap-3 text-xs"><div><p className="uppercase text-zinc-600">Desconto</p><p className="mt-1 font-mono text-lg font-black text-brand">{partner.discountPercentage}%</p></div><div><p className="uppercase text-zinc-600">Criado em</p><p className="mt-1 text-zinc-300">{dateFormatter.format(new Date(partner.createdAt))}</p></div></div><div className="border border-white/8 bg-black p-3"><p className="truncate font-mono text-[11px] text-zinc-500">{buildPartnerLink(publicOrigin, partner.slug)}</p><button type="button" onClick={onCopy} className="mt-2 inline-flex items-center gap-2 text-xs font-black uppercase text-brand">{copied ? <Check className="h-4 w-4" /> : <Clipboard className="h-4 w-4" />}{copied ? 'Copiado' : 'Copiar link'}</button></div><ActionButtons partner={partner} onEdit={onEdit} onStatus={onStatus} onDelete={onDelete} /></article>;
}
