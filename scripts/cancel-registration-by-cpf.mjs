import { createHash, randomUUID } from 'node:crypto';
import { existsSync, readFileSync } from 'node:fs';

function loadEnvFile(path) {
  if (!existsSync(path)) {
    return;
  }

  for (const line of readFileSync(path, 'utf8').split(/\r?\n/)) {
    const trimmed = line.trim();

    if (!trimmed || trimmed.startsWith('#')) {
      continue;
    }

    const separatorIndex = trimmed.indexOf('=');

    if (separatorIndex < 0) {
      continue;
    }

    const key = trimmed.slice(0, separatorIndex).trim();
    let value = trimmed.slice(separatorIndex + 1).trim();

    if ((value.startsWith('"') && value.endsWith('"')) || (value.startsWith("'") && value.endsWith("'"))) {
      value = value.slice(1, -1);
    }

    process.env[key] = value;
  }
}

function cpfHash(cpf) {
  return createHash('sha256').update(cpf.replace(/\D/g, '')).digest('hex');
}

loadEnvFile('.env');
loadEnvFile('.env.local');

const cpf = process.argv[2];

if (!cpf) {
  throw new Error('Usage: node scripts/cancel-registration-by-cpf.mjs <cpf>');
}

const { transaction } = await import('../server/database.ts');
const hash = cpfHash(cpf);

const result = await transaction((database) => {
  const now = new Date().toISOString();
  const registrations = database.registrations.filter((registration) => (
    registration.cpfHash === hash && ['pending_payment', 'paid'].includes(registration.status)
  ));

  for (const registration of registrations) {
    const previousStatus = registration.status;
    registration.status = 'cancelled';
    registration.updatedAt = now;
    registration.expiresAt = registration.expiresAt || now;

    const payment = database.payments.find((item) => item.registrationId === registration.id);

    if (payment && ['pending_payment', 'paid'].includes(payment.status)) {
      payment.status = 'cancelled';
      payment.updatedAt = now;
      payment.expiresAt = payment.expiresAt || now;
    }

    if (['pending_payment', 'paid'].includes(previousStatus)) {
      const lot = database.lots.find((item) => item.id === registration.lotId);

      if (lot && lot.soldCount > 0) {
        lot.soldCount -= 1;

        if (lot.status === 'sold_out' && lot.soldCount < lot.capacity) {
          lot.status = 'active';
        }
      }
    }

    database.auditLogs.push({
      id: randomUUID(),
      actor: 'admin',
      action: 'registration.cancelled_for_retest',
      entityType: 'registration',
      entityId: registration.id,
      payload: {
        previousStatus,
        paymentId: payment?.id || null,
        reason: 'Manual cancellation for checkout retest.',
      },
      createdAt: now,
    });
  }

  return {
    provider: process.env.DATABASE_PROVIDER || (process.env.DATABASE_URL ? 'postgres' : 'json'),
    cancelled: registrations.map((registration) => ({
      id: registration.id,
      status: registration.status,
      amountCents: registration.amountCents,
      updatedAt: registration.updatedAt,
    })),
  };
});

console.log(JSON.stringify(result, null, 2));
